...
async def ask_support(message: types.Message):
    ...
    keyboard = await support.support_keyboard(messages="one")
    await message.answer(text, reply_markup=keyboard)


@dp.callback_query_handler(support.support_callback.filter(messages="one"))
async def send_to_support(call: types.CallbackQuery, state: FSMContext, callback_data: dict):
    await call.answer()
    user_id = int(callback_data.get("user_id"))
    await call.message.answer("Пришлите текст в следующем сообщении")
...
async def get_support_message(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(second_id, f"Вам письмо! Вы можете ответить нажав на кнопку ниже")
    keyboard = await support.support_keyboard(messages="one", user_id=message.from_user.id)
